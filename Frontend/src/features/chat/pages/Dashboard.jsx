...
const Dashboard = () => {
  ...
  const handleLogout = async () => {
    try {
      await fetch("https://perplexity-a4uh.onrender.com/api/auth/logout", {
        method: "POST",
        credentials: "include",
      });
    } catch (err) {
  ...
  return (
    <main
      className="relative flex h-screen w-full overflow-hidden bg-[#0b0c0c] text-[#ece9e1]"
    ...
    >
      ...
      >
        ...
        >
          ...
          <p className="mb-2 text-[12px] font-medium uppercase tracking-wide text-white/30">
            Recents
          </p>
          ...
          <div className="flex-1 space-y-1 overflow-y-auto pb-2">
            ...
            {sortedChats.length === 0 && (
              <p className="px-3.5 py-2 text-[14px] text-white/25">
                No chats yet
              </p>
            )}
          </div>
          ...
          <div
            className="relative border-t border-white/[0.07] pt-4"
            ref={menuRef}
          >
            {menuOpen && (
              <div className="absolute bottom-[calc(100%+10px)] left-0 w-[260px] rounded-2xl border border-white/[0.09] bg-[#1c1d1c] p-2 shadow-[0_16px_50px_rgba(0,0,0,0.55)]">
                {email && (
                  <p className="truncate px-3.5 py-2.5 text-[13.5px] text-white/40">
                    {email}
                  </p>
                )}
    ...
    `,
        ...
        {currentMessages.length === 0 ? (
          <div className="relative z-10 flex h-full flex-col items-center justify-center px-6">
            ...
            <p className="mb-12 text-[16px] text-white/40">
              Ask anything, backed by real-time search.
            </p>
                      ...
                      <div className="text-[16.5px] leading-[1.7] text-white/90">
                        <ReactMarkdown
                          components={{
                            ...
                            ul: ({ children }) => (
                              <ul className="mb-3.5 list-disc space-y-2 pl-5">
                                {children}
                              </ul>
                            ),
                            ol: ({ children }) => (
                              <ol className="mb-3.5 list-decimal space-y-2 pl-5">
                                {children}
                              </ol>
                            ),
                            strong: ({ children }) => (
                            ...
                            code: ({ children }) => (
                              <code className="rounded-md bg-white/[0.09] px-1.5 py-0.5 text-[14.5px] text-white/95">
                                {children}
                              </code>
                            ),
                            pre: ({ children }) => (
                              <pre className="mb-3.5 overflow-x-auto rounded-xl border border-white/[0.07] bg-black/40 p-4">
                                {children}
                              </pre>
                            ),
                          }}
